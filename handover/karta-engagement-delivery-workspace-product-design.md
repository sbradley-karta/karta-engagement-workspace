...
AI strategy weekly status deck, September 10, 2026 (Sean: a new deck with one clean status slide and a drafted status for the AI strategy plan). `docs/slides/build-status-deck.js` writes `docs/slides/ai-strategy-status-2026-09-12.pptx`, one slide on the Karta palette with generous spacing: title and week, an overall pill (Green, with the note that one workstream is Yellow), three columns (accomplished this week, planned next week, decisions, risks and watch items with small kind tags), a five-row workstream table with status pills (Engagement Workspace · Project Status Reporting, Foundations Package, Governance and operating model, Platform accounts and IT as Yellow, Skills and practitioner enablement) and a 90-day strip (Sep 12 status 1, Oct 3, Oct 31, Dec 5 releases, Dec 12 scale review, marked as proposed dates to confirm). Content drawn from the handover record and the SWE Asana project (gid 1215524577812860) as of September 10: complete skills Estimator, Win Story, Industry One Pager, RFP Questions, Status Decks, RAID Log updates, Granola to Asana follow-ups; in progress BRDs, Design Deck, Integration Matrix (Connor), Anaplan UX constraints (Sean), Asana project creation, RAID Log and Status Decks skills (Ethan). Decisions asked of Will and Paul: confirm release dates; name the second pilot engagement and its reviewer. Risk: Entra admin consent pending. The content arrays at the top of the script are edited week to week; the full narrative is in the speaker notes. Validated and geometry-checked; not rendered (no LibreOffice).
